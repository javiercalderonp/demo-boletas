...
class GCSStorageServiceSignedUrlTests(unittest.TestCase):
    ...
    def test_generate_signed_url_resolves_default_compute_account_during_refresh(self):
        service = self._build_service()
        blob = Mock()
        blob.generate_signed_url.side_effect = [
            AttributeError("you need a private key to sign credentials"),
        ...
        def refresh(_request):
            credentials.service_account_email = "runtime@example.iam.gserviceaccount.com"
        ...
        self.assertEqual(second_call_kwargs["access_token"], "runtime-token")
